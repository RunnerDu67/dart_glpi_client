...
class GlpiService {
  ...
  Future<Map<String, dynamic>> getItem(String itemType, int id) async {
    final response = await http.get(
  ...
  Future<Map<String, dynamic>> addItem({
    ...
    if (response.statusCode == 201) return json.decode(response.body); // 201 Created
  ...
  Future<void> deleteItem(String itemType, int id) async {
    final response = await http.delete(
      Uri.parse('$apiUrl/$itemType/$id'),
      headers: _getHeaders(),
      // L'API GLPI attend un corps vide ou un `input` pour la suppression
      body: json.encode({'input': {'id': id}})
    );
  ...
  Map<String, String> _getHeaders() {
    if (!isSessionActive) {
      throw Exception('Session non initialisée. Appelez initSession() d\'abord.');
    }
    return {
      'Content-Type': 'application/json',
